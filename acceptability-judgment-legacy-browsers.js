...
let expName = 'acceptability-judgment';  // from the Builder filename that created this script
let expInfo = {
...
async function experimentInit() {
  ...
  slider = new visual.Slider({
    ...
    size: [1, 0.1], pos: [0, 0], ori: 0.0, units: psychoJS.window.units,
    labels: ["1\ntotally\nunacceptable", 2, 3, 4, "5\nperfectly\nacceptable"], fontSize: 0.05, ticks: [1, 2, 3, 4, 5],
...
function trialRoutineBegin(snapshot) {
  return async function () {
    ...
    slider.reset()
    // Run 'Begin Routine' code from registerResponse
    ...
    trialComponents.push(proceed_key);
...
function trialRoutineEachFrame() {
  return async function () {
    ...
    if (t >= 0.0 && proceed_key.status === PsychoJS.Status.NOT_STARTED) {
      ...
      // keyboard checking is just starting
      proceed_key.clock.reset();
      proceed_key.start();
      proceed_key.clearEvents();
    }
    
    // if proceed_key is active this frame...
    ...
    // Run 'Each Frame' code from registerResponse
    var _pj;
    ...
    if (error) {
        errorBox.text = "Please make a selection before proceeding.";
    } else {
        errorBox.text = "";
    }
